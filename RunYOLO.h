...
//#include <iostream> //Tracker.h
#include <ImageProcessing.h>
#include <typeinfo> //only here
#include <queue>    //only here
#include <iterator> //only here
#include <vector>   //only here
#include <sstream>  //only here
#include <fstream>  //only here
#include <iomanip>  //only here
#include <chrono>   //only here

//#include <opencv2/core.hpp>             //Tracker.h
#include <opencv2/dnn.hpp>              //only here
#include <opencv2/dnn/all_layers.hpp>   //only here

//#include <opencv2/imgproc.hpp>          //Tracker.h Form_VideoFrame.h
//#include <opencv2/highgui.hpp>          //Tracker.h Form_VideoFrame.h

//using namespace cv;                     //Form_VideoFrame.h
//using namespace std;                    //Form_VideoFrame.h

class RunYOLOonCPU {
public:

    const cv::Scalar colors = { 0, 0, 255 };
    std::tuple<cv::Mat,int, std::vector<cv::Rect>,std::vector<float>> output_tuple;
    ...
    *pnet   - ?????? ?? ??????????? ? ??????????? ?????? ???? - ?????? ?????? OpenCV dnn
    ...
    ?????, ??? ?????????? ??????? ????????, ?????????????? ? main(), ???????? ?????? ??????? ? C++17
    */
        std::vector<std::string> output_names = (*pnet).getUnconnectedOutLayersNames();
        ...
        output_tuple = make_tuple(frame,obj_number,boxes[0],scores[0]);
        return output_tuple;
    }
};
